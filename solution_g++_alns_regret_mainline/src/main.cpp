...
private:
    ...
    Solution rebuildWithRemoved(
        ...
        if (allow_regret && params.regret_repair_enabled > 0 && params.regret_repair_k >= 2 &&
            ...
            int regret_k = max(2, params.regret_repair_k);
            while (!remaining.empty()) {
                ...
                for (int idx = 0; idx < static_cast<int>(remaining.size()); ++idx) {
                    ...
                    double priority = params.regret_repair_weight * regret - best_cost;
    ...
    vector<int> selectSmallWindowJobs(const Solution &solution, int iter, mt19937 &rng) const {
        int n = static_cast<int>(jobs.size());
        if (n <= 1) {
            return {};
        }
        ...
        if (static_cast<int>(result.size()) < q) {
            vector<int> fallback = selectRemovedJobs(solution, iter % 5, rng);
    ...
    void improveLocally(
        ...
        for (int iter = 0; iter < total_iterations; ++iter) {
            ...
            bool window_iter = iter >= standard_iterations;
            int mode = (probability(rng) < params.random_repair_ratio) ? 5 : (iter % 5);
            vector<int> original_removed = window_iter
                ? selectSmallWindowJobs(current_solution, iter - standard_iterations, rng)
            ...
            if (isBlockTabu(block_signature, block_tabu, iter)) {
                ...
                for (int attempt = 1; attempt <= 4; ++attempt) {
                    int alternate_mode = (mode + attempt) % 6;
                    vector<int> alternate_removed =
                        filterTabuJobs(selectRemovedJobs(current_solution, alternate_mode, rng), job_tabu_until, iter);
            ...
            bool allow_regret = params.regret_repair_enabled > 0 && params.regret_repair_k >= 2;
            ...
            if (allow_regret && params.regret_repair_max_block > 0 &&
                static_cast<int>(removed.size()) > params.regret_repair_max_block) {
                allow_regret = false;
            }

            Solution candidate = rebuildWithRemoved(current_solution, removed, repair_strategy, allow_regret);
            bool accepted = false;
            if (candidate.valid) {
                if (candidate.score < best_solution.score - 1e-7) {
                    accepted = true;
